                ...
                {links.map(({ href, label, children, Icon }) => {
                    ...
                    if(children) {
                        return <div key={href} className="flex flex-col">
                            <div onClick={() => setExtended(!extended)} className={`px-4 py-2 hover:bg-gray-100 rounded-lg transition-colors flex flex-row items-center text-sm font-medium space-x-2 ${router.asPath == href ? 'bg-blue-100 text-blue-600': 'text-neutral-700'}`}>
                                <Icon className="w-5 h-5"/> <span>{label}</span>
                            </div>
                            <Collapse in={extended}>
                                <div className="flex flex-col pl-4">
                                    {children.map(({ href, label, Icon }) => {
                                        return <div key={href} className={`px-4 py-2 hover:bg-gray-100 rounded-lg transition-colors flex flex-row items-center text-sm font-medium space-x-2 ${router.asPath == href ? 'bg-blue-100 text-blue-600': 'text-neutral-700'}`}>
                                            <Icon className="w-5 h-5"/> <span>{label}</span>
                                        </div>
                                    })}
                                </div>
